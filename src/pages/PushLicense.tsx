...
export default function PushLicense() {
  ...
  const handleDevicePasswordError = (
    messagePasswordSuccess: boolean,
    passwordErrorTruerFalse: boolean,
    messageHideTrueOrFalse: boolean,
    text: string
  ) => {
  ...
  const handleDevicePathdError = (
    messagePathSuccess: boolean,
    pathErrorTruerFalse: boolean,
    messageHideTrueOrFalse: boolean,
    text: string
  ) => {
  ...
  const handleDeviceIPError = (
    messageIpSuccess: boolean,
    ipErrorTruerFalse: boolean,
    messageHideTrueOrFalse: boolean,
    text: string
  ) => {
  ...
  const handelLicenseUploadSuccess = () => {
    ...
    handleDevicePasswordError(
      true,
      false,
      false,
      "Pod with is restarting and the license file is succesfully applied"
    );
  };
  ...
  const handlePushClick = async () => {
    if (deviceIPInput.length <= 0) {
      handleDeviceIPError(
        ...
        true,
        false,
        "Please enter a IP address of the Pod and try again."
      );
    ...
    if (devicePasswordInput.length <= 0) {
      handleDevicePasswordError(
        ...
        true,
        false,
        "Please enter a Pods password try again."
      );
      handleDeviceIPError(true, false, false, "");
      handleDevicePathdError(true, false, false, "");
      return;
    ...
    if (pathToLicenseInput.length <= 0) {
      handleDevicePathdError(
        false,
        true,
        false,
        "Please select .bin licesne file and try again."
      );
      handleDeviceIPError(true, false, false, "");
      handleDevicePasswordError(true, false, false, "");
      return;
    ...
    const serverURL =
      "https://license-web-service-foln5hyksa-ew.a.run.app/upload";
    ...
    try {
      ...
      if (responseBody.message === "timeout of 5000ms exceeded") {
        handleDeviceIPError(
          ...
          true,
          false,
          "Cannot connect to the Pod! Please make sure your computer can reach Pod, and the device IP address is correct"
        );
        handleDevicePasswordError(true, false, false, "");
        handleDevicePathdError(true, false, false, "");
      }
      ...
      if (responseBody.message === "Unable to apply license file change") {
        handleDevicePathdError(
          ...
          true,
          false,
          "Invalid license file. Please make sure the license file is correct."
        );
        handleDeviceIPError(true, false, false, "");
        handleDevicePasswordError(true, false, false, "");
      }
  ...
  return (
    <div className="card">
      <h2>Push License</h2>
      <p>
        Utilize this tool to push one license file to a Mersive Pod running
        Solstice version 5.5.3 or higher.
      </p>

      <div className="input-icons">
        <input
          className={`input-field device-field-error-${deviceIPError}`}
          type="text"
          placeholder="Pod IP address"
          value={deviceIPInput}
            ...
            title={
              <div>
                ...
                <p style={{ color: "white", fontSize: "small" }}>
                  The address starts with http:// and is usually located at the
                  top right of the room display.
                </p>
              </div>
            }
            placement="top"
      ...
      <div className="input-icons">
        <input
          className={`input-field device-field-error-${devicePasswordError}`}
          placeholder="Pod password  (if set)"
        ...
        <span onClick={handlePasswordVisability}>
          {passwordVisability ? (
            <VisibilityIcon
              className={`icon icon-error-${devicePasswordError}`}
            />
          ) : (
            <VisibilityOffIcon
              className={`icon icon-error-${devicePasswordError}`}
      ...
      {loading ? (
        ...
        <button className={`submit-button`} onClick={handlePushClick}>
          Push Licese File
        </button>
      )}
